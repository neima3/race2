// halo-flats authoring probe: geo metrics + autopilot laps (no-ring baseline = laps.ts
// ground truth; ring-boosted lap mimics the main.ts hit logic) + per-ring hit check.
...
const id = process.argv[2] ?? 'halo-flats';
...
console.log(
  `${id}: len=${curve.length.toFixed(0)}m frames=${n} y=[${minY.toFixed(1)}..${maxY.toFixed(1)}] ` +
    `kink=${((kinkMax * 180) / Math.PI).toFixed(1)}deg@d${fr[kinkAt].dist.toFixed(0)} grade=${(maxGrade * 100).toFixed(0)}%@d${fr[Math.max(0, gradeAt)].dist.toFixed(0)} bank=${((maxBank * 180) / Math.PI).toFixed(0)}deg`,
);
...
// --- ring world positions (makeGateRing math) ---
const ringCenters = (def.rings ?? []).map((r) => {
...
// --- autopilot lap loop (laps.ts ground truth) ---
function runLap(ringBoosts: boolean): { finished: boolean; timeMs: number; minSpeed: number; respawns: number; ringHits: number[] } {
  ...
  const closest = ringCenters.map(() => 99);
  const latAtRing = ringCenters.map(() => 99);
  ...
  while (wall < 150000) {
    ...
    wall += simDt * 1000;
    if (ringBoosts) {
      for (let ri = 0; ri < ringCenters.length; ri++) {
        if (Math.abs(car.state.trackDist - ringCenters[ri].dist) < 1.5) {
          latAtRing[ri] = Math.abs(car.state.lateral) < Math.abs(latAtRing[ri]) ? car.state.lateral : latAtRing[ri];
          if (process.argv.includes('--dbg')) console.log(`    [r${ri}] d=${car.state.trackDist.toFixed(1)} lat=${car.state.lateral.toFixed(2)} y=${car.state.pos.y.toFixed(2)} dist=${car.state.pos.distanceTo(ringCenters[ri].center).toFixed(2)}`);
        }
        if (hit.has(ri)) continue;
        const d = car.state.pos.distanceTo(ringCenters[ri].center);
        if (d < ringCenters[ri].radius) {
          hit.add(ri);
          car.applyBoost(7, 1.2);
        }
        closest[ri] = Math.min(closest[ri], d);
      }
    }
    ...
    if (Math.floor(car.state.trackDist / 50) !== Math.floor((car.state.trackDist - car.state.speed * simDt) / 50)) {
  ...
  if (ringBoosts) {
    console.log(
      `  rings: ${hit.size}/${ringCenters.length} hit | per-ring closest: ` +
        ringCenters.map((r, i) => `#${i}@d${r.dist} ${closest[i].toFixed(2)}m lat≈${latAtRing[i].toFixed(1)} (r${r.radius})`).join(' | '),
    );
  }
  return { finished, timeMs: Math.round(race.elapsedMs), minSpeed, respawns, ringHits: [...hit], speedLog };
}

const base = runLap(false);
console.log(
  base.finished
    ? `  LAP(no-ring) PASS ${(base.timeMs / 1000).toFixed(2)}s minSpeed=${base.minSpeed.toFixed(1)}m/s respawns=${base.respawns}`
    : '  LAP(no-ring) FAIL: DID NOT FINISH',
);
console.log(`  speed(km/h) every 50m: ${base.speedLog.map((p) => `${p.d}:${p.s}`).join(' ')}`);
const ringed = runLap(true);
console.log(
  ringed.finished
    ? `  LAP(rings)   PASS ${(ringed.timeMs / 1000).toFixed(2)}s (−${(base.timeMs - ringed.timeMs)}ms vs no-ring) respawns=${ringed.respawns}`
    : '  LAP(rings)   FAIL: DID NOT FINISH',
);
